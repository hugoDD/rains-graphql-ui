<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      <el-form-item label="表名称" prop="re_tableName">
        <el-input
      ...
      <el-form-item label="表描述" prop="re_tableComment">
        <el-input
...
import { genBaseMutation } from '../../../operation/genCode'
export default {
  name: "Gen",
  ...
  data() {
    return {
      ...
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        re_tableName: undefined,
        re_tableComment: undefined
      },
      // 预览参数
      preview: {
        open: false,
  ...
  methods: {
    ...
    getList() {
      ...
      listTable(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
          this.tableList = response.data.pages.data;
    ...
    handleGenTable(row) {
      ...
      this.$graphqlDownload(genBaseMutation, {request: {...reqParam}}, `rains_code.zip`)
    ...
    handlePreview(row) {
      previewTable(row.tableId).then(response => {
        this.preview.data = response.data.genTablePage.data[0].preview
