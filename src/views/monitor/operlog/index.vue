<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      <el-form-item label="系统模块" prop="re_operation">
        <el-input
          v-model="queryParams.re_operation"
          placeholder="请输入系统模块"
          clearable
          style="width: 240px;"
          size="small"
      ...
      <el-form-item label="操作人员" prop="re_username">
        <el-input
...
<!--      <el-table-column label="请求方式" align="center" prop="requestMethod" />-->
    ...
    <el-dialog title="操作日志详细" :visible.sync="open" width="700px">
      <el-form ref="form" :model="form" label-width="100px" size="mini">
        <el-row>
          <el-col :span="12">
            <el-form-item label="操作模块：">{{ form.operation }} / 修改</el-form-item>
            <el-form-item
              label="登录信息："
            >{{ form.username }} / {{ form.ip }} / {{ form.location }}</el-form-item>
          </el-col>
          <el-col :span="12">
...
import { objToRsql, requestParam } from '../../../utils'
import { initLogsQuery } from '../../../operation/log'
...
export default {
  ...
  created() {
    ...
    req.filter = objToRsql(this.queryParams)
    this.loading = true
    this.$graphql(initLogsQuery,{request:req}).then(response => {
      this.list = response.data.pages.data
      this.total = response.data.pages.total
      this.typeOptions = response.data.sysOperType.data
      this.statusOptions = response.data.sysCommonStatus.data
      this.loading = false;
    })
